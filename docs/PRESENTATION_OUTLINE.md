...
**Title:** E-commerce Search Algorithm Comparison: Keyword Matching vs. TF-IDF on Real Marketplace Data

**Course:** COMP 5112 (Research Methodology in Computer Science)  
**Department:** Computer Science / Faculty of Science and Environmental Studies  
...
- **Data Diversity:** Structured API data vs. unstructured social media content
- **Algorithm Selection:** Need to understand which approaches work best for different data types
...
### Research Question
**"How do keyword matching and TF-IDF algorithms compare for e-commerce search using real marketplace data?"**
...
- **Example:** "Just bought this amazing coffee maker from Breville, totally worth the $200!"
...
### Relevance Judgments
- **Synthetic Approach:** Created using keyword overlap and exact match criteria
...
*[Display 2x2 grid of graphs showing F1-Score, Precision, Recall, and NDCG trends]*

### F1-Score Trends (K=1 to 10)
- **TF-IDF (Pink Line):** Strong upward trend from ~0.04 (K=1) to ~0.15 (K=10)
- **Keyword Matching (Blue Line):** Minimal improvement from ~0.005 (K=1) to ~0.03 (K=10)
...
### Summary
**TF-IDF consistently and significantly outperforms Keyword Matching across all metrics on social media data, demonstrating its effectiveness in handling noisy, unstructured text.**
...
- **Key Insight:** Both algorithms perform better on structured data, but TF-IDF still dominates

### Recall Trends (K=1 to 10)
- **TF-IDF:** Strong upward trend from ~0.03 (K=1) to ~0.15 (K=10)
- **Keyword Matching:** Minimal improvement from ~0.005 (K=1) to ~0.02 (K=10)
- **Key Insight:** Structured data improves both algorithms, but TF-IDF advantage remains

### NDCG Trends (K=1 to 10)
- **TF-IDF:** Consistently high, ~0.95 (K=1) to ~0.93 (K=10)
- **Keyword Matching:** Lower performance, ~0.38 (K=1) to ~0.44 (K=5-10)
- **Key Insight:** TF-IDF provides superior ranking quality across all data types

### Summary
**TF-IDF maintains superior performance on both structured and unstructured data, demonstrating its robustness across different data sources.**
...
#### TF-IDF Advantages
- **Semantic Understanding:** Crucial for handling natural language variations
...
**TF-IDF generally provides superior performance for e-commerce search, especially when dealing with diverse and unstructured real-world data like social media posts.**
...
- **Algorithm Behavior Analysis:** Understanding how algorithms handle different query types
...
- **TF-IDF Superiority:** Consistently outperforms Keyword Matching across all metrics
...
#### Advanced Algorithms
- **Transformer Models:** BERT, Sentence Transformers for deeper semantic understanding
...
### Final Thoughts
**Effective search is paramount for navigating the vast landscape of online product information. This research provides a foundation for understanding and improving e-commerce search systems.**
